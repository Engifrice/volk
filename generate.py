...
if __name__ == "__main__":
    ...
    blocks["VERSION_DEFINE"] = (
        "#define VOLK_HEADER_VERSION " + version.find("name").tail.strip() + "\n"
    )
    ...
    for feature in spec.findall("feature"):
        ...
        name = re.sub(
            r"VK_(BASE|COMPUTE|GRAPHICS)_VERSION_", "VK_VERSION_", name
        )  # strip Vulkan Base prefixes for compatibility
        key = defined(name)
